# ------------------------------------------------
# Gretina Lite Geant4 Simulation
...
Geometry is a simplified version of Gretina, with the aluminium chamber, beam tubes, and spherical shell.
One module is made of a clover of hexagonal tapered Ge crystals with copper contacts on the back, and an aluminium
back end. The single module is rotated and translated according to slot 0 in the UCGretina sim:
https://github.com/rileyle/UCGretina
...
and a nTuple. The only filter for what's tallied is a check that the logical volume that the particle is in
has the name corresponding to the Germanium volume.
...
The file name of the output .root file is chosen in runAction.cc. Code is in place to output 'GretLiteTest.root' or to make
a filename based on the system time. Uncomment the one you want.

Code is based on examples basic/B1 and basic/B4/B4a and previous sims of mine. The dimensions of the geometry were taken
from the UCGretina simulation, or from papers by S. Paschalis et al.
...
./GretinaLite       - brings up GUI. In the GUI command bar do /control/execute runMacroVis.mac to use the macro
...
./GretinaLite name-of-macro.mac             - executes in batch mode (no GUI so quicker) according to instructions given in macro.
