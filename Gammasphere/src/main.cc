...
int main(int argc, char** argv){
    // Look at input args, none -> boot GUI, one (a macro name) -> run in batch (terminal only) mode
    ...
    G4RunManager * runManager = G4RunManagerFactory::CreateRunManager(G4RunManagerType::Default);
        ...
        G4int nThreads = 10; // Change number of threads here, I can make nThreads a command line argument if needed.
    ...
    G4HadronicParameters::Instance()->SetTimeThresholdForRadioactiveDecay(1.0e60*CLHEP::year); // needed in newer G4 versions
