...
OurRunAction::OurRunAction()
{
    ...
    G4double EMin = 0.;
    G4double EMax = 8*MeV;
    // First histogram, index 0 automatically
    analysisManager->CreateH1("EDep Hist", "Pulse height spectrum tally histogram. Analogue to gamma spec", bins, EMin, EMax); // name, title, nbins, xmin, xmax
    // Second hist, index 1 automatically
    analysisManager->CreateH1("Bg EDep Hist", "Background PHST. Counts from events that entered Ge and BGO", bins, EMin, EMax); // name, title, nbins, xmin, xmax
    // 3rd
    analysisManager->CreateH1("Geom-eff", "Log of misses, Ge, and BGO hits", 3, 0., 3.); // name, title, nbins, xmin, xmax

    // First 2D histogram
    analysisManager->CreateH2("EGam-ModNum", "Gamma energy vs GS module number", bins, EMin, EMax, 110, 0., 110.);
    // 2nd
    analysisManager->CreateH2("Bg-EGam-ModNum", "Gamma energy vs GS module number", bins, EMin, EMax, 110, 0., 110.);
...
void OurRunAction::BeginOfRunAction(const G4Run*)
{
    ...
    G4String filename = "GSphere" + G4String(timeStr) + ".root";
    // G4String filename = "GSphereTest.root";
