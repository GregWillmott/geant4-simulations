...
void OurEventAction::EndOfEventAction(const G4Event*)
{
    ...
    // (2) Ge and BGO hit
        ...
        analysisManager->FillH2(1, GeEventEDep, GS_ID);
    }
    ...
    // Efficiency estimate
    if((if_Ge_hit == true) && (if_BGO_hit == false)){ // Ge hit only. Success
        analysisManager->FillH1(2,1.5);
    }
    else if(if_BGO_hit == true){ // BGO hit at some point. Failure 1
        analysisManager->FillH1(2,2.5);
    }
    else{
        analysisManager->FillH1(2,0.5); // Nothing hit. Failure 2
    }
}

void OurEventAction::AddEDep(G4double dE, G4String hitVol, G4int vol_ID, G4RotationMatrix* rot){
